...
async function loadOptions() {
    try {
        ...
        if (info.ok) {
            ...
            document.getElementById('badge-txt').textContent = md.model + ' | R2 = ' + md.r2_score;
...
async function predict() {
    ...
    try {
        ...
        animateNum(document.getElementById('pv'), d.predict_price);
        document.getElementById('p-lo').textContent = fmt(d.price_low);
        document.getElementById('p-hi').textContent = fmt(d.price_high);
        document.getElementById('s-r2').textContent = (d.r2_score * 100).toFixed(1) + ' %';
        ...
        err.textContent = e.message + ' | Make sure Flask API is running (python api/app.py)';
